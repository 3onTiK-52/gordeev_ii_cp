...
class CAnyMovable {
  ...
  template <class T> using CObjType = std::remove_reference_t<T>;
...
public:
  ...
  template <class TObject, class... TArgs> void emplace(TArgs &&...args) {
    pIObject_ =
...
private:
  ...
  template <class TObject> class CObjectKeeper : public IObjectStored {
  public:
